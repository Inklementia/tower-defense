...
int Menu::run(SDL_Renderer *renderer) {
  ...
  while (running) {
    ...
    HudRenderer::drawCheckerboard(
        renderer, GameConfig::WINDOW_WIDTH, GameConfig::WINDOW_HEIGHT,
        GameConfig::TILE_SIZE, UiColor{46, 46, 52, 255},
        UiColor{32, 32, 36, 255});

    HudRenderer::drawTextCentered(renderer, GameConfig::WINDOW_WIDTH / 2, 48,
                                  "Minimalistic Tower Defense",
                                  UiColor{255, 196, 64, 255}, 42);

    for (int i = 0; i < 2; i++) {
      int bx = (GameConfig::WINDOW_WIDTH - 440) / 2;
      int by = 180 + i * (64 + 14);
      bool hovered = mouseX >= bx && mouseX <= bx + 440 && mouseY >= by &&
                     mouseY <= by + 64;

      HudRenderer::drawPanel(renderer, bx, by, 440, 64,
                             hovered ? UiColor{48, 54, 72, 255}
                                     : UiColor{36, 40, 54, 255});
      HudRenderer::drawPanel(renderer, bx, by, 440, 3,
                             UiColor{255, 196, 64, 255});
    ...
    HudRenderer::drawTextCentered(renderer, GameConfig::WINDOW_WIDTH / 2, 420,
                                  "Press 1 or 2, or click a mode",
                                  UiColor{150, 154, 168, 255}, 16);
    HudRenderer::drawTextCentered(renderer, GameConfig::WINDOW_WIDTH / 2, 442,
                                  "ESC to quit", UiColor{150, 154, 168, 255},
                                  16);
