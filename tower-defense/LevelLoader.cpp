...
LevelSettings LevelLoader::load(const std::string& filename) {
	...
	while (std::getline(file, line)) {
		...
		if (key == "target") {
			...
			settings.allowWallEdit = (value == "1" || value == "true" || value == "yes");
		}
		else if (key == "turrets_on_walls_only") {
			settings.turretsOnWallsOnly = (value == "1" || value == "true" || value == "yes");
		}
		else if (key == "show_flow_arrows") {
			settings.showFlowArrows = (value == "1" || value == "true" || value == "yes");
		}
		else if (key == "creative_mode") {
			settings.creativeMode = (value == "1" || value == "true" || value == "yes");
		}
	}
