...
int main(int argc, char *args[]) {
  // Seed the random number generator with the current time so that it will
  // generate different numbers every time the game is run.
  srand((unsigned)time(NULL));

  if (SDL_Init(SDL_INIT_VIDEO | SDL_INIT_AUDIO) < 0) {
    std::cout << "Error: Couldn't initialize SDL Video or Audio = "
              << SDL_GetError() << std::endl;
    return 1;
  } else {
    // Setup the audio mixer.
    bool isSDLMixerLoaded =
        (Mix_OpenAudio(GameConfig::AUDIO_FREQUENCY, MIX_DEFAULT_FORMAT,
                       GameConfig::AUDIO_CHANNELS,
                       GameConfig::AUDIO_CHUNK_SIZE) == 0);
    if (isSDLMixerLoaded == false) {
      std::cout << "Error: Couldn't initialize Mix_OpenAudio = "
    ...
    SDL_Window *window = SDL_CreateWindow(
        "Tower Base Defense", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        GameConfig::WINDOW_WIDTH, GameConfig::WINDOW_HEIGHT, 0);
    if (window == nullptr) {
      std::cout << "Error: Couldn't create window = " << SDL_GetError()
                << std::endl;
      return 1;
    } else {
      ...
      if (renderer == nullptr) {
        std::cout << "Error: Couldn't create renderer = " << SDL_GetError()
                  << std::endl;
        return 1;
      } else {
        ...
        while (appRunning) {
          ...
          LevelSettings settings =
              LevelLoader::load(LevelLoader::getLevelPath(levelIndex));
