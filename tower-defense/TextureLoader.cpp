...
std::unordered_map<std::string, SDL_Texture *>
    TextureLoader::umapTexturesLoaded;

SDL_Texture *TextureLoader::loadTexture(SDL_Renderer *renderer,
                                        std::string filename) {
  if (filename != "") {
    ...
    if (found != umapTexturesLoaded.end()) {
      ...
      // Setup the relative filepath to the images folder using the input
      // filename.
