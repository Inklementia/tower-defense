...
bool LevelSaver::saveLayout(const std::string& filename, int targetX, int targetY,
	const std::vector<std::pair<int, int>>& spawners,
	const std::vector<std::pair<int, int>>& walls) {
	...
	if (!writeLayoutFile(filename, preservedLines, targetX, targetY, spawners, walls)) {
		std::cout << "Error: Could not save level file " << filename << std::endl;
		return false;
	}

	if (!sourcePath.empty() && sourcePath != normalizePath(filename)) {
		if (!writeLayoutFile(sourcePath, preservedLines, targetX, targetY, spawners, walls)) {
			std::cout << "Warning: Could not save source level file " << sourcePath << std::endl;
		}
	}
