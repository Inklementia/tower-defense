...
void drawStringBitmap(SDL_Renderer* renderer, int x, int y, const std::string& text,
	UiColor color, int fontSize) {
	if (!fontLoaded || text.empty())
		return;
	...
	for (unsigned char c : text) {
		...
		unsigned char* bitmap = stbtt_GetCodepointBitmap(&fontInfo, scale, scale, c, &gw, &gh, &xoff, &yoff);
			...
			if (surface != nullptr) {
				...
				for (int row = 0; row < gh; row++) {
					for (int col = 0; col < gw; col++) {
						...
						pixels[(size_t)row * gw + col] = SDL_MapRGBA(surface->format,
...
void HudRenderer::drawText(SDL_Renderer* renderer, int x, int y, const std::string& text,
	UiColor color, int fontSize) {
	drawStringBitmap(renderer, x, y, text, color, fontSize);
}

void HudRenderer::drawTextCentered(SDL_Renderer* renderer, int centerX, int y, const std::string& text,
	UiColor color, int fontSize) {
	drawStringBitmap(renderer, centerX - measureGlyphRunWidth(text, fontSize) / 2, y, text, color, fontSize);
}

void HudRenderer::drawPanel(SDL_Renderer* renderer, int x, int y, int w, int h, UiColor color) {
...
void HudRenderer::drawCheckerboard(SDL_Renderer* renderer, int width, int height, int tileSize,
